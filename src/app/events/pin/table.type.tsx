...
export const columns = [
  ...
  columnHelper.accessor(
    (row) => row.dailyEvents.map((dailyEvent) => dailyEvent.dailyEventDate),
    {
      header: '날짜',
      cell: ({ getValue }) => {
        ...
        return (
          <div className="flex h-full flex-col justify-between">
            {dates.map((date, index) => (
              <p
                key={index}
                className="flex h-[58px] grow items-center justify-center whitespace-nowrap break-keep border-b border-basic-grey-200 px-8 last:border-b-0"
              >
