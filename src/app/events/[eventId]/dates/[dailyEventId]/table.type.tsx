...
export const getColumns = ({
  ...
  const cheerCampaignDiscountRate =
    cheerCampaign?.result?.finalDiscountRate ?? 0;
  const hasCheerCampaignDiscount =
    cheerCampaign?.result && cheerCampaignDiscountRate > 0;
  ...
  return [
    ...
    columnHelper.display({
      ...
      cell: (info) => {
        ...
        const toDestinationArrivalTime =
          toDestinationDestinationHub?.arrivalTime
      ...
      cell: (info) => {
        ...
        return (
          <div className="flex flex-col items-center gap-[6px]">
            <span className={`${style[statusText]} font-500`}>
              {statusText}
            </span>
            <span className="text-12 font-500 text-basic-grey-600">
              {reservationDeadline &&
    ...
    columnHelper.display({
      ...
      cell: (info) => {
        ...
        const basePriceRoundTrip = isEarlybirdPeriod
          ? earlybirdPriceRoundTrip
          : regularPriceRoundTrip;
        const basePriceToDestination = isEarlybirdPeriod
          ? earlybirdPriceToDestination
          : regularPriceToDestination;
        const basePriceFromDestination = isEarlybirdPeriod
          ? earlybirdPriceFromDestination
          : regularPriceFromDestination;

        const finalPriceRoundTrip = hasCheerCampaignDiscount
          ? calculateFinalPrice(basePriceRoundTrip, cheerCampaignDiscountRate)
          : basePriceRoundTrip;
        const finalPriceToDestination = hasCheerCampaignDiscount
          ? calculateFinalPrice(
              basePriceToDestination,
              cheerCampaignDiscountRate,
            )
          : basePriceToDestination;
        const finalPriceFromDestination = hasCheerCampaignDiscount
          ? calculateFinalPrice(
              basePriceFromDestination,
              cheerCampaignDiscountRate,
            )
          : basePriceFromDestination;

        const formattedFinalPriceRoundTrip = finalPriceRoundTrip
          ? `${finalPriceRoundTrip.toLocaleString()}원`
          : '-';
        const formattedFinalPriceToDestination = finalPriceToDestination
          ? `${finalPriceToDestination.toLocaleString()}원`
          : '-';
        const formattedFinalPriceFromDestination = finalPriceFromDestination
          ? `${finalPriceFromDestination.toLocaleString()}원`
          : '-';

        if (isEarlybirdPeriod || hasCheerCampaignDiscount) {
          return (
            <div className="flex flex-col gap-[6px]">
              <div
              ...
              >
                ...
                <span className="font-500">{formattedFinalPriceRoundTrip}</span>
              </div>
              <div
              ...
              >
                ...
                <span className="font-500">
                  {formattedFinalPriceToDestination}
                </span>
              </div>
              <div
              ...
              >
                ...
                <span className="font-500">
                  {formattedFinalPriceFromDestination}
                </span>
              </div>
              {isEarlybirdPeriod && (
                <span className="text-center text-12 font-500 text-basic-grey-600">
                  얼리버드 진행 중 ~
                  {formatDateString(earlybirdDeadline, 'date')}
                </span>
              )}
              {hasCheerCampaignDiscount && (
                <span className="text-center text-12 font-500 text-basic-grey-600">
                  응원 캠페인 할인 {cheerCampaignDiscountRate}% 적용
                </span>
              )}
            </div>
          );
          ...
          <div className="flex flex-col gap-[6px]">
            <div className="grid grid-cols-2 items-center gap-x-16">
              ...
              <div className="flex items-center gap-8">
                {hasCheerCampaignDiscount && (
                  <span
                    className={`text-12 font-500 text-basic-grey-600 line-through ${isReservationDisabledToDestination ? 'opacity-30' : ''}`}
                  >
                    {formattedRegularPriceToDestination}
                  </span>
                )}
              ...
              <div className="flex items-center gap-8">
                {hasCheerCampaignDiscount && (
                  <span
                    className={`text-12 font-500 text-basic-grey-600 line-through ${isReservationDisabledFromDestination ? 'opacity-30' : ''}`}
                  >
                    {formattedRegularPriceFromDestination}
                  </span>
                )}
              ...
              <div className="flex items-center gap-8">
                {hasCheerCampaignDiscount && (
                  <span
                    className={`text-12 font-500 text-basic-grey-600 line-through ${isReservationDisabledRoundTrip ? 'opacity-30' : ''}`}
                  >
                    {formattedRegularPriceRoundTrip}
                  </span>
                )}
